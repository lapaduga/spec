...
$(document).ready(function () {
	...
	$('.header__burger').on('click', function () {
		$('.header__burger, .menu, .header').toggleClass('_active');
	...
	$('.tabs__trigger').on('click', function (e) {
		...
		$('.tabs__item').fadeOut(0);
		$(e.target).addClass('tabs__trigger--active');
		$($(e.target).attr('href')).fadeIn(300);
	});
	$('.tabs__trigger--all').on('click', function () {
		$('.tabs__item').fadeIn(300);
	});

	// Маска телефона
	$("#form__input--phone").mask("+7 (999) 999-99-99");
	$("#feedback__input--phone").mask("+7 (999) 999-99-99");
	$("#about__input--phone").mask("+7 (999) 999-99-99");

	// Валидация формы на странице КОНТАКТЫ
	var name = document.getElementById('form__input--name');
	var phone = document.getElementById('form__input--phone');
	var form = document.getElementById('content-contacts__form');
	var errorElement = document.getElementById('form__error');
	...
	if (form) {
		form.addEventListener('submit', function (e) {
			if (name.value === '' || name.value == null) {
				...
				name.classList.add('form__imput--error');
			} else {
				name.classList.remove('form__imput--error');
			}
			if (phone.value === '' || phone.value == null) {
				e.preventDefault();
				errorElement.classList.add('form__error--visible');
				phone.classList.add('form__imput--error');
			} else {
				phone.classList.remove('form__imput--error');
			}
		});
	}

	// Обертка таблиц
	var serviceBody = document.querySelector('.service__body');
